...
const STATUS = {
  ...
  unreachable: {
    dot:     '#EA4335',  // red
    label:   'Servicio de notificaciones caído',
    pulse:   true,
  },
}

const NOTICE_URL = import.meta.env.VITE_NOTICE_SYSTEM_URL 
  || 'http://localhost:3001'
        ...
        <div style={{
          ...
          {(status === 'disconnected' || 
            ...
            <div style={{
              ...
              {status === 'waiting_qr' 
                ? '→ Escanea el QR en localhost:3001/qr'
                : status === 'unreachable'
                ? '→ El servicio de notificaciones no responde'
                : '→ WhatsApp desconectado. Revisa el servicio.'
              }
            </div>
          )}
